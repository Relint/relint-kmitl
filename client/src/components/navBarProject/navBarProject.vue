              ...
              chat = chat.map((value, index) => {
                ...
                if (
                  ...
                  if (!this.lastIndex) {
                    //First in-line
                    if (
                      !res.read.includes(firebase.auth().currentUser.uid) &&
                      !this.checkUser(res.uid)
                    ) {
                      res.prompt = true;
                      res.msg = "Unread messages below";
                      this.lastIndex = index;
                      document.getElementById("unread-btn").style.display =
                        "block";
                    }
                  } else {
                    //Next in-line
  ...
  methods: {
    ...
    sendMessage() {
      ...
      ref.get().then(doc => {
        ...
        chat = chat.map(value => {
          if (
            !value.read.includes(firebase.auth().currentUser.uid) &&
            !this.checkUser(value.uid)
          ) {
      ...
      if (document.getElementsByClassName("relative-msg-border")) {
        ...
        this.messages = this.messages.map((ele, index) => {
          if (index !== 0 && index + 1 !== this.messages.length) {
            ...
            this.relativeScrollHeight +=
              document.getElementsByClassName("relative-msg-border")[index]
                .scrollHeight + 5;
          }
          ele.relativeScrollHeight = this.relativeScrollHeight;
          ele.height = document.getElementsByClassName("relative-msg-border")[
            index
          ].scrollHeight;
          return ele;
        });
    ...
    goUnread() {
      if (this.unreadIndex.length > 0) {
        ...
        document.getElementById("chatBox").scrollTop =
          this.messages[this.unreadIndex[0]].relativeScrollHeight -
          this.messages[this.unreadIndex[0]].height * 2;
    ...
    wheelie() {
      if (this.lastIndex) {
        const relativeHeight = this.messages[this.lastIndex]
          .relativeScrollHeight;
